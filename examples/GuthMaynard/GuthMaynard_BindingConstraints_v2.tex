...
\title{Binding Constraints After Guth--Maynard:\\
A Structural Analysis of the EXPDB Pipeline}
...
We work within the EXPDB framework of Tao, Trudgian, and Yang~\cite{TTY25}.  The master polytope $\mathcal{P} \subset \mathbb{R}^{5}$ lives in coordinates $(\sigma, \tau, \rho, \rho^{*}, s)$.  The pipeline descends:
\[
  \mathcal{P} \;\xrightarrow{\;\pi\;}\; R_{\mathrm{LV}} \subset \mathbb{R}^{3}
  \;\xrightarrow{\;\sup\;}\; A(\sigma)
  \;\xrightarrow{\;\sup_{\sigma}\;}\; \|A\|_{\infty}
  \;\xrightarrow{\;\text{Ingham}\;}\; \theta.
\]

The zero-density function $A(\sigma)$ satisfies $N(\sigma, T) \le T^{A(\sigma) + \varepsilon}$, where $N(\sigma, T)$ counts zeros of $\zeta(s)$ with real part $\ge \sigma$ and imaginary part $\le T$.  The prime gap exponent is
\[
  \theta_{\mathrm{PNTALL}} \le 1 - \frac{1}{\|A\|_{\infty}},
  \qquad
  \|A\|_{\infty} := \sup_{1/2 \le \sigma \le 1} A(\sigma).
\]

The mean-square gap exponent satisfies
\[
  \theta_{\mathrm{GAPSQUARE}} \le \max\!\left(
    2 - \frac{2}{\|A\|_{\infty}},\;
    \sup_{\sigma} \max\bigl(\alpha(\sigma),\, \beta(\sigma)\bigr)
  \right),
\]
where $\alpha(\sigma) = 4\sigma - 2 + 2(B(\sigma)(1-\sigma) - 1)/(B(\sigma) - A(\sigma))$, $\beta(\sigma) = 4\sigma - 2 + (B(\sigma)(1-\sigma) - 1)/A(\sigma)$, and $B(\sigma) \ge A^{*}(\sigma)$ is the zero-density energy exponent.

\section{Updated Exponents}
...
\[
  \|A\|_{\infty} = \frac{30}{13} \approx 2.3077,
  \qquad
  \sigma^{*} = \frac{7}{10},
\]
\[
...
\[
  \Delta\theta = \frac{7}{12} - \frac{17}{30}
  = \frac{35 - 34}{60} = \frac{1}{60} \approx 0.0167.
\]

\section{The Cusp at \texorpdfstring{$\sigma = 7/10$}{sigma = 7/10}}
...
\paragraph{Left limb.}  For $\sigma \in [1/2, 7/10)$, the binding estimate is Ingham~\cite{Ing40}:
\[
  A_{\mathrm{Ing}}(\sigma) = \frac{3}{2 - \sigma}.
\]
This is an increasing, concave-up function of $\sigma$.

\paragraph{Right limb.}  For $\sigma \in [7/10, 19/25)$, the binding estimate is Guth--Maynard~\cite{GM24}:
\[
...
\[
  A_{\mathrm{Ing}}(7/10) = \frac{3}{2 - 7/10} = \frac{30}{13},
  \qquad
  A_{\mathrm{GM}}(7/10) = \frac{15}{5 \cdot 7/10 + 3} = \frac{30}{13}.
\]
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/A_sigma_envelope.png}
  \caption{The zero-density envelope $A(\sigma)$ computed from 482 EXPDB hypotheses.  Each colored segment indicates the binding constraint on that interval.  Dashed curves show the Ingham and GM bounds extended beyond their binding ranges.  The cusp at $\sigma^{*} = 7/10$ with $A = 30/13$ is marked.}
  \label{fig:envelope}
\end{figure}

\section{Binding and Slack Constraints}

\subsection*{Binding at $\sigma^{*}$}
...
\midrule
1 & Guth--Maynard~\cite{GM24} & $[7/10,\,19/25)$ & $30/13$ & \textbf{Binding} \\
2 & Ingham~\cite{Ing40}       & $[1/2,\,7/10)$   & $30/13$ & \textbf{Binding} \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection*{Locally binding (away from $\sigma^{*}$)}
...
\midrule
Ivi\'c (2003)~\cite{Ivi03}        & $[19/25,\sim\!0.78)$        & $\sim 2.21$ \\
Tao--Trudgian--Yang (2025)~\cite{TTY25} & $[\sim\!0.78,\sim\!0.90)$ & $\sim 2.01$ \\
Ivi\'c (1980)~\cite{Ivi80}        & $[0.80,0.88)$               & $\sim 1.88$ \\
Derived (Bourgain EP)~\cite{Bou17}& $[\sim\!0.92,\sim\!0.98)$   & $\sim 1.09$ \\
Pintz (2023)~\cite{Pin23}         & $[\sim\!0.96,1)$            & $\sim 0.71$ \\
\bottomrule
\end{tabular}
\end{center}

\subsection*{Completely slack}
...
\midrule
Huxley (1972)~\cite{Hux72}        & $12/5 = 2.400$ & Superseded by GM everywhere \\
Montgomery (1971)~\cite{Mon71}    & $5/2 = 2.500$  & Superseded \\
Heath-Brown (1979) ZD~\cite{HB79zd} & $9/4 = 2.250$ & Superseded by Ivi\'c/TTY \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection*{Sensitivity of $\theta_{\mathrm{PNTALL}}$}

The Ingham relation $\theta = 1 - 1/\|A\|_{\infty}$ gives
\[
  \frac{d\theta}{d\|A\|_{\infty}}
  = \frac{1}{\|A\|_{\infty}^{2}}
  = \frac{13^{2}}{30^{2}} = \frac{169}{900} \approx 0.1878.
\]
...
\subsection*{Sensitivity of $\theta_{\mathrm{GAPSQUARE}}$}

At the binding point $\sigma \approx 0.999$ (where $\alpha$ is maximized), the numerically computed sensitivities are:
\[
  \frac{\partial\theta_{\mathrm{GS}}}{\partial A}\bigg|_{\sigma \approx 1} \approx -0.133,
  \qquad
  \frac{\partial\theta_{\mathrm{GS}}}{\partial B}\bigg|_{\sigma \approx 1} \approx +0.134.
\]
...
\section{The Role of $A^{*}(\sigma)$}

\paragraph{For $\theta_{\mathrm{PNTALL}}$.}  The Ingham relation involves only $A(\sigma)$, not $A^{*}(\sigma)$.  Improving the energy exponent has zero effect on the prime gap bound.  The bottleneck is entirely in the large-value projection $\pi_{\sigma,\tau,\rho}(\mathcal{P})$.

\paragraph{For $\theta_{\mathrm{GAPSQUARE}}$.}  The Heath-Brown mean-square formula involves both $A(\sigma)$ and $B(\sigma) \ge A^{*}(\sigma)$.  After Guth--Maynard, the current binding energy estimate is
\[
  A^{*}(\sigma) \le \frac{12}{4\sigma - 1} \qquad \text{on } [5/6,\,1),
\]
from Heath-Brown~\cite{HB79zd}.  At $\sigma \approx 0.999$, this gives $A^{*} \approx 4.01$ while $A \approx 0.14$, yielding $B/A \approx 28$.  The function $\alpha(\sigma)$ dominates $\beta(\sigma)$ in this regime.
...
\subsection*{How GM reshaped $A(\sigma)$}

In the $(\sigma, A(\sigma))$ plane, the best zero-density envelope has the shape of an asymmetric arch:
\begin{itemize}
  \item \textbf{Rising limb} ($\sigma \in [1/2, \sigma^{*}]$): governed by Ingham, $A = 3/(2-\sigma)$.
  \item \textbf{Peak} ($\sigma = \sigma^{*}$): the cusp at $\|A\|_{\infty}$.
  \item \textbf{Falling limb} ($\sigma \in [\sigma^{*}, 1)$): governed by a sequence of estimates --- GM, then Ivi\'c, then TTY, then Bourgain-derived, then Pintz --- each tighter than the last as $\sigma$ increases.
\end{itemize}
...
\subsection*{How GM reshaped $\mathcal{P}$}

At the level of the 5D polytope $\mathcal{P}$, Guth--Maynard contributes:
\begin{enumerate}
  \item \textbf{Three LV half-spaces} in $(\sigma, \tau, \rho)$: the constraint $\rho \le \max(2 - 2\sigma,\; 18/5 - 4\sigma,\; \tau + 12/5 - 4\sigma)$.
  \item \textbf{Twenty iterated LV half-spaces} (parameter $K = 1, \ldots, 20$), giving progressively tighter bounds via the Guth--Maynard iteration.
  \item \textbf{Three LVER half-spaces} in full 5D, coupling $\rho$, $\rho^{*}$, and $s$ via Lemma 10.18 of Guth--Maynard~\cite{GM24}.
\end{enumerate}
...
\section{Roadmap for the Next Improvement}

The cusp structure at $\sigma^{*} = 7/10$ determines the attack surface.

\subsection*{Path A: Beat GM to the right of $\sigma = 7/10$}
...
\subsection*{Path B: Beat Ingham to the left of $\sigma = 7/10$}
...
\begin{tabular}{clccl}
\toprule
Priority & Target & $\theta_{\mathrm{PNTALL}}$ & $\theta_{\mathrm{GAPSQUARE}}$ & Difficulty \\
\midrule
...
We verified computationally that the EXPDB pipeline has fully exploited the Guth--Maynard result for $\theta_{\mathrm{PNTALL}}$; no pipeline optimization changes the binding value $30/13$.  The sole identified gap (stale exponent pairs in \texttt{bourgain\_ep\_to\_zd}) affects only the $\theta_{\mathrm{GAPSQUARE}}$ computation and has been reported to the EXPDB maintainers.

\section{Fine-Grained Sensitivity Map}

\subsection*{The effective radius}

The Ingham relation $\theta = 1 - 1/\|A\|_{\infty}$ concentrates all sensitivity into a narrow band around $\sigma^{*}$.  Define the \emph{headroom} at $\sigma$ as $h(\sigma) := \|A\|_{\infty} - A(\sigma)$.  An improvement of magnitude $\varepsilon$ in $A$ at $\sigma$ affects $\theta$ only if $h(\sigma) < \varepsilon$.

The following table gives the \emph{effective radius} --- the interval of $\sigma$ values within headroom $\varepsilon$ of the peak --- for several improvement magnitudes:
...
\subsection*{Cusp geometry and attack surface}

At the cusp $\sigma^{*} = 7/10$, the two binding curves have derivatives:
\[
  \frac{d}{d\sigma}\!\left[\frac{3}{2 - \sigma}\right]\bigg|_{\sigma = 7/10}
  = \frac{3}{(13/10)^{2}} = \frac{300}{169} \approx +1.775,
\]
\[
  \frac{d}{d\sigma}\!\left[\frac{15}{5\sigma + 3}\right]\bigg|_{\sigma = 7/10}
  = \frac{-75}{(13/2)^{2}} = \frac{-300}{169} \approx -1.775.
\]

The slopes are \emph{exactly symmetric}: $|dA_{\mathrm{Ing}}/d\sigma| = |dA_{\mathrm{GM}}/d\sigma| = 300/169$.  This means the cusp is a $V$-shaped notch with opening angle $2\arctan(169/300) \approx 58^{\circ}$.  To lower $\|A\|_{\infty}$ by $\varepsilon$, it suffices to beat the current bound on an interval of width
\[
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/sensitivity_heatmap.png}
  \caption{Sensitivity $d\theta_{\mathrm{PNTALL}}/dA(\sigma)$ as a function of $\sigma$ (horizontal) and improvement magnitude $\varepsilon$ (vertical).  The cone of nonzero sensitivity flares from the cusp at $\sigma = 0.70$.  White boundary lines trace the $V$-shaped contour with slopes $\pm 300/169$.  For small $\varepsilon$, only $\sigma \in [0.695, 0.705]$ contributes.}
  \label{fig:sensitivity_heatmap}
\end{figure}

\subsection*{Sensitivity map for $\theta_{\mathrm{GAPSQUARE}}$}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/alpha_sensitivity.png}
  \caption{Partial derivatives $\partial\alpha/\partial A$ (blue) and $\partial\alpha/\partial B$ (red) as functions of $\sigma$.  The crossover at $\sigma \approx 0.751$ separates $A$-dominant (left) from $B$-dominant (right) regimes.  The zero crossing of $\partial\alpha/\partial A$ at $\sigma \approx 0.804$ marks where improving $A$ becomes counterproductive for $\theta_{\mathrm{GAPSQUARE}}$.}
  \label{fig:alpha_sensitivity}
\end{figure}

Note that $\partial\alpha/\partial A$ changes sign near $\sigma \approx 0.80$.  For $\sigma > 0.80$, \emph{decreasing} $A$ actually \emph{increases} $\alpha$ (and hence worsens $\theta_{\mathrm{GAPSQUARE}}$).  This is because the $\alpha$ formula has $A$ in the denominator $B - A$: decreasing $A$ widens the gap $B - A$, which decreases $\alpha$ through the numerator but increases it through the denominator.  The net sign depends on the local ratio $B/A$.

\subsection*{The succession landscape}
...
2nd           & $\sim 0.76$    & $\sim 2.21$           & $0.10$ & Ivi\'c (2003)         \\
3rd           & $\sim 0.77$    & $\sim 2.09$           & $0.22$ & Ivi\'c (1979) / TTY   \\
4th           & $\sim 0.80$    & $\sim 1.88$           & $0.43$ & Ivi\'c (1980)         \\
...
After the first improvement past GM, the second improvement would need to target a \emph{different} $\sigma$ range ($\sigma \approx 0.76$) and a \emph{different} constraint family (Ivi\'c, not GM).  The targets march rightward along $\sigma$ as the peak is lowered, each requiring a distinct analytical technique.  Reaching the Density Hypothesis $\|A\|_{\infty} = 2$ requires beating \emph{every} bound in the table --- a sequence of progressively harder problems spanning the entire range $\sigma \in (1/2, 1)$.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/succession_bottlenecks.png}
  \caption{Successive hypothetical improvements to the $A(\sigma)$ envelope.  Black: current envelope.  Red ($\varepsilon = 0.10$): first shave exposes Ivi\'c (2003) at $\sigma \approx 0.76$.  Orange ($\varepsilon = 0.22$): second shave exposes Ivi\'c (1979)/TTY.  Purple ($\varepsilon = 0.43$): third shave reaches Ivi\'c (1980) at $\sigma \approx 0.80$.  The bottleneck migrates rightward with each improvement.}
...
\bibitem{TTY25}
T.~Tao, T.~Trudgian, A.~Yang,
\emph{New exponent pairs, zero density estimates, and zero additive energy estimates: a systematic approach},
arXiv:2501.16779, 2025.

\bibitem{Ing40}
A.~E.~Ingham,
\emph{On the estimation of $N(\sigma, T)$},
Quarterly Journal of Mathematics (Oxford) \textbf{11} (1940), 291--292.
...
Inventiones Mathematicae \textbf{15} (1972), 164--170.

\bibitem{HB79zd}
D.~R.~Heath-Brown,
\emph{Zero density estimates for the Riemann zeta-function and Dirichlet L-functions},
Journal of the London Mathematical Society (2) \textbf{19} (1979), 221--232.

\bibitem{Ivi80}
A.~Ivi\'c,
\emph{Exponent pairs and the zeta function of Riemann},
Studia Scientiarum Mathematicarum Hungarica \textbf{15} (1980), 157--181.
...
Journal of the American Mathematical Society \textbf{30}(1) (2017), 205--224.

\bibitem{Pin23}
J.~Pintz,
\emph{On the density of zeros of the Riemann zeta-function},
preprint, 2023.
